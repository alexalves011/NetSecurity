...
const sensitivityLevels = [
  {
    id: "low",
    label: "Low",
    desc: "Menos alertas, mais flexível",
    badge: "Baixa",
  ...
  },
  {
    id: "medium",
    label: "Medium",
    desc: "Equilíbrio entre segurança e usabilidade",
    badge: "Média",
  ...
  },
  {
    id: "high",
    label: "High",
    desc: "Máxima segurança, mais alertas",
    badge: "Alta",
...
export default function SettingsPage() {
  ...
  const [sessionTimeout, setSessionTimeout] = useState("8 horas");

  return (
    <DashboardLayout>
      <PageHeader
        title="Configurações & IAM"
        subtitle="Gerenciamento de usuários, funções e sensibilidade da IA"
      />
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Ajuste o nível de detecção de anomalias comportamentais
              </p>
            </div>
            ...
            {sensitivityLevels.map((level) => (
              ...
              >
                <div className="flex items-center gap-3">
                  <div
                    className={`h-4 w-4 rounded-full border-2 flex items-center justify-center ${sensitivity === level.id ? "border-primary" : "border-muted-foreground/40"}`}
                  >
                    {sensitivity === level.id && (
                ...
                <span
                  className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${level.badgeColor}`}
                >
                  {level.badge}
                </span>
              </button>
        ...
        <div className="dashboard-card">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Preferências de segurança e notificações
              </p>
            </div>
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-accent text-accent-foreground">
              <Settings size={20} />
          ...
          <div className="space-y-5">
            <div className="flex items-center justify-between">
              ...
              >
                <span
                  className={`absolute top-0.5 h-5 w-5 rounded-full bg-card shadow transition-transform ${mfa ? "left-[22px]" : "left-0.5"}`}
                />
              </button>
            </div>
            <div className="flex items-center justify-between">
              ...
              >
                <span
                  className={`absolute top-0.5 h-5 w-5 rounded-full bg-card shadow transition-transform ${emailNotifications ? "left-[22px]" : "left-0.5"}`}
                />
              </button>
            </div>
            <div>
            ...
            <tbody>
              {users.map((user) => (
                <tr key={user.email} className="border-t border-border">
                  ...
                  <td className="py-4">
                    <span
                      className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${statusColors[user.status]}`}
                    >
                      {user.status}
                    </span>
                  </td>
                  ...
                  <td className="py-4">
                    <span
                      className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${sensitivityColors[user.sensitivity]}`}
                    >
                      {user.sensitivity}
                    </span>
                  </td>
